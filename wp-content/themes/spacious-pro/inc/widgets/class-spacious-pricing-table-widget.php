...
/**
 * Class spacious_pricing_table_widget.
 */
class spacious_pricing_table_widget extends WP_Widget {

	function __construct() {
		$widget_ops  = array(
			'classname'   => 'widget_table_pricing',
			'description' => esc_html__( 'Use this widdget to show the Pricing Table plan. Best for Business Top or Bottom sidebar.', 'spacious' ),
		);
		$control_ops = array( 'width' => 200, 'height' => 250 );
		parent::__construct( false, $name = esc_html__( 'TG: Pricing Table', 'spacious' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'spacious' ); ?></label>
		...
		<p>
			<?php esc_html_e( 'Note: Enter number of columns for Pricing Table (default 3 columns) and save it then enter the data in respective field.', 'spacious' ); ?>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'col_num' ); ?>" class="widefat"><?php esc_html_e( 'Display no of Columns:', 'spacious' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'col_num' ); ?>" name="<?php echo $this->get_field_name( 'col_num' ); ?>">
				<option value="2" <?php echo esc_attr( '2' == $col_num ? 'selected="selected"' : '' ); ?> ><?php esc_html_e( 'Two', 'spacious' ); ?></option>
				<option value="3" <?php echo esc_attr( '3' == $col_num ? 'selected="selected"' : '' ); ?>><?php esc_html_e( 'Three', 'spacious' ); ?></option>
				<option value="4" <?php echo esc_attr( '4' == $col_num ? 'selected="selected"' : '' ); ?>><?php esc_html_e( 'Four', 'spacious' ); ?></option>
			</select>
		</p>
		<hr />
		<?php for ( $col_count = 0; $col_count < $instance['col_num']; $col_count ++ ) : ?>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_name_' . $col_count ); ?>"><?php esc_html_e( 'Title :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_price_' . $col_count ); ?>"><?php esc_html_e( 'Price :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_desc_' . $col_count ); ?>"><?php esc_html_e( 'Description :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_subtitle_' . $col_count ); ?>"><?php esc_html_e( 'Sub Title :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_badge_' . $col_count ); ?>"><?php esc_html_e( 'Badge :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_badge_bg_' . $col_count ); ?>"><?php esc_html_e( 'Badge background :', 'spacious' ); ?></label><br />
			...
			<p>
				<?php esc_html_e( 'Note: Enter the number of features to display in column (default 4 features) and save it then enter the data in respective field.', 'spacious' ); ?>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id( 'package_feature_num_' . $col_count ); ?>"><?php esc_html_e( 'Number of features to display :', 'spacious' ); ?></label>
			...
			<p><?php esc_html_e( 'Features :', 'spacious' ); ?></p>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_btn_txt_' . $col_count ); ?>"><?php esc_html_e( 'Button Text :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_btn_url_' . $col_count ); ?>"><?php esc_html_e( 'Button URL :', 'spacious' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'package_color_' . $col_count ); ?>"><?php esc_html_e( 'Color :', 'spacious' ); ?></label><br />
	...
	function widget( $args, $instance ) {
		...
		if ( function_exists( 'icl_register_string' ) && ! empty( $text ) ) {
			icl_register_string( 'Spacious Pro', 'TG: Pricing Table widget description text' . $this->id, $text );
		}
		...
		for ( $col_count = 0; $col_count < $col_num; $col_count ++ ) {
			...
			if ( function_exists( 'icl_register_string' ) && ! empty( $packages ) ) {
				icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Title' . $this->id . $col_count, $packages[ $col_count ] );
				if ( ! empty( $prices ) ) {
					icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Price' . $this->id . $col_count, $prices[ $col_count ] );
				}
				if ( ! empty( $package_descs ) ) {
					icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Description' . $this->id . $col_count, $package_descs[ $col_count ] );
				}
				if ( ! empty( $subtitles ) ) {
					icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Sub Title' . $this->id . $col_count, $subtitles[ $col_count ] );
				}
				if ( ! empty( $badge ) ) {
					icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Badge' . $this->id . $col_count, $badge[ $col_count ] );
				}
				if ( ! empty( $btn_text ) ) {
					icl_register_string( 'Spacious Pro', 'TG: Pricing Table Package Button Text' . $this->id . $col_count, $btn_text[ $col_count ] );
				}
				if ( ! empty( $features ) ) {
					foreach ( $features[ $col_count ] as $key => $value ) {
						icl_register_string( 'Spacious Pro', 'TG: Pricing Table Table Features' . $this->id . $col_count . $key, $value );
					}
				}
			}
		}
		...
		<div class="section-wrapper">
			...
			<div class="tg-container pricing-table pricing-column-<?php echo esc_attr( $col_num ); ?> pricing-table-layout-<?php echo esc_attr( $layout ); ?> clearfix">
				<?php if ( ! empty( $title ) ) : ?>
				...
				<?php if ( ! empty( $text ) ) : ?>
					<p class="widget-desc"><?php
						...
						if ( function_exists( 'icl_t' ) ) {
							$text = icl_t( 'Spacious Pro', 'TG: Pricing Table widget description text' . $this->id, $text );
						}
				...
				<?php if ( ! empty( $packages ) ) : ?>
					...
					<?php for ( $col_count = 0; $col_count < $col_num; $col_count ++ ) :
						...
						<div class="<?php echo esc_attr( $column_class ) ?>">
							<?php if ( ! empty( $colors ) || ! empty( $badge_bg ) ) {
								$dark_color = spacious_darkcolor( $colors[ $col_count ], - 20 ); ?>
								<!-- Widget CSS -->
								<style type="text/css">
									<?php echo '#' . esc_attr( $this->id ); ?>
									.pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-title,
									<?php echo '#' . esc_attr( $this->id ); ?> .pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-price,
									<?php echo '#' . esc_attr( $this->id ); ?> .pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-btn a {
										background: <?php echo esc_attr( $colors[ $col_count ] ) ?>;
									}

									<?php echo '#' . esc_attr( $this->id ); ?>
									.pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-price:after {
										border-top-color: <?php echo esc_attr( $colors[ $col_count ] ) ?>;
									}

									<?php echo '#' . esc_attr( $this->id ); ?>
									.pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-subtitle {
										color: <?php echo esc_attr( $colors[ $col_count ] ) ?>;
									}

									<?php echo '#' . esc_attr( $this->id ); ?>
									.pricing-table-layout-2 .pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-title,
									<?php echo '#' . esc_attr( $this->id ); ?> .pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-btn a:hover {
										background: <?php echo esc_attr( $dark_color ) ?>;
									}

									<?php echo '#' . esc_attr( $this->id ); ?>
									.pricing-table-wrapper-<?php echo absint( $col_count ); ?> .pricing-as-popular {
										background: <?php echo esc_attr( $badge_bg[ $col_count ] ) ?>;
									}
								</style>
							<?php } ?>
							...
							<?php if ( $packages[ $col_count ] != '' ) : ?>
								<div class="pricing-table-wrapper-<?php echo absint( $col_count ); ?> pricing-table-wrapper">
									<?php if ( $badge[ $col_count ] ) :
										// For WPML plugin compatibility
										if ( function_exists( 'icl_t' ) ) {
											$badge[ $col_count ] = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Badge' . $this->id . $col_count, $badge[ $col_count ] );
										} ?>
										<span class="pricing-as-popular"><?php echo esc_html( $badge[ $col_count ] ); ?></span>
									<?php endif; ?>

									<?php // For WPML plugin compatibility
									if ( function_exists( 'icl_t' ) ) {
										$packages[ $col_count ] = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Name' . $this->id . $col_count, $packages[ $col_count ] );
									} ?>
									<?php if ( ! empty( $packages[ $col_count ] ) ) : ?>
										<h4 class="pricing-title"><?php echo esc_html( $packages[ $col_count ] ); ?></h4>
									<?php endif; ?>

									<?php // For WPML plugin compatibility
									if ( function_exists( 'icl_t' ) ) {
										$prices[ $col_count ]        = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Price' . $this->id . $col_count, $prices[ $col_count ] );
										$package_descs[ $col_count ] = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Description' . $this->id . $col_count, $package_descs[ $col_count ] );

									} ?>

									<?php if ( ! empty( $packages[ $col_count ] ) || ! empty( $prices[ $col_count ] ) || ! empty( $package_descs[ $col_count ] ) ) : ?>
										<div class="pricing-price">
											<span class="pricing-currnecy"><?php echo esc_html( $prices[ $col_count ] ); ?></span><span class="pricing-date"><?php echo esc_html( $package_descs[ $col_count ] ); ?></span>
										</div>
									<?php endif; ?>
									<?php if ( ! empty( $feature_num ) || ! empty( $subtitles[ $col_count ] ) ) : ?>
										<ul class="pricing-list-wrapper">
											<?php // For WPML plugin compatibility
											if ( function_exists( 'icl_t' ) ) {
												$subtitles[ $col_count ] = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Subname' . $this->id . $col_count, $subtitles[ $col_count ] );
											}
											if ( $subtitles[ $col_count ] != '' ) : ?>
												<span class="pricing-subtitle"><?php echo esc_html( $subtitles[ $col_count ] ); ?></span>
											<?php endif; ?>
											<?php if ( ! empty( $features[ $col_count ] ) ) :
												foreach ( $features[ $col_count ] as $key => $value ) {
													if ( $value != '' ) {
														// For WPML plugin compatibility
														if ( function_exists( 'icl_t' ) ) {
															$value = icl_t( 'Spacious Pro', 'TG: Pricing Table Table Features' . $this->id . $col_count . $key, $value );
														}
									...
									<?php // For WPML plugin compatibility
									if ( function_exists( 'icl_t' ) ) {
										$btn_text[ $col_count ] = icl_t( 'Spacious Pro', 'TG: Pricing Table Package Button Text' . $this->id . $col_count, $btn_text[ $col_count ] );
									} ?>

									<?php if ( ! empty( $btn_text[ $col_count ] ) ) : ?>
										<div class="pricing-btn">
											<a href="<?php echo esc_url( $btn_URL[ $col_count ] ); ?>"><?php echo esc_html( $btn_text[ $col_count ] ); ?></a>
										</div>
									<?php endif; ?>
