...
class Spacious_Admin_Notice {
	...
	public function tdi_notice() {
		...
		<div class="error updated tdi-notice" style="position:relative;">
			...
			$button_label = esc_html__( 'Update Now', 'spacious' );

			$msg = sprintf(
				/* Translators: 1: Plugin name */
				'<p>' . esc_html__( 'Action Required: Please update "%1$s" plugin to latest version to make sure your site is all secure. We released a security patch in the latest version of this plugin.', 'spacious' ) . '</p>',
			...
			$msg .= sprintf(
				/* Translators: 1: Plugin name */
				'<p style="display: inline-block; margin-left: 5px;">' . esc_html__( 'Also, if the purpose of importing the demo via "%1$s" plugin is fulfilled, you can simply delete this plugin now.', 'spacious' ) . '</p>',
				'<strong>' . esc_html__( 'ThemeGrill Demo Importer', 'spacious' ) . '</strong>'
			);
