...
class JCK_WSSV {
	...
	private function load_classes() {
		...
		add_action( 'init', array( 'Iconic_WSSV_Compat_FacetWP', 'init' ), 10 );
	...
	public function save_product_variation( $variation_id, $i ) {
		$data = array(
			'visibility'          => array(),
			'add_to_cart'         => $i !== false ? isset( $_POST['jck_wssv_variable_disable_add_to_cart'][ $i ] ) : Iconic_WSSV_Product_Variation::get_add_to_cart( $variation_id ),
			'featured_visibility' => $i !== false ? isset( $_POST['jck_wssv_variable_featured'][ $i ] ) : Iconic_WSSV_Product_Variation::get_featured_visibility( $variation_id ),
			'listings_only'       => $i !== false ? isset( $_POST['jck_wssv_variable_listings_only'][ $i ] ) : Iconic_WSSV_Product_Variation::get_listings_only( $variation_id ),
			'title'               => $i !== false && isset( $_POST['jck_wssv_display_title'][ $i ] ) ? $_POST['jck_wssv_display_title'][ $i ] : '',
		);
	...
	public function add_attributes_to_variation( $variation_id, $i = false, $force = false ) {
		$show_in_filtered = isset( $_POST['jck_wssv_variable_show_filtered'][ $i ] ) && $_POST['jck_wssv_variable_show_filtered'][ $i ] == "on";
