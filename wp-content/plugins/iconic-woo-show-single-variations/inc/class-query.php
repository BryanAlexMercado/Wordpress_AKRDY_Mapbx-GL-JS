...
class Iconic_WSSV_Query {
	 ...
	 */
	public static function add_variations_to_product_query( $q, $wc_query = false ) {
		$post_type = array_filter( (array) $q->get( 'post_type' ) );

		if ( ( ! in_array( 'product', $post_type ) && empty( $wc_query ) ) || ( ! is_woocommerce() || ! $q->is_main_query() || empty( $q->query_vars['wc_query'] ) ) ) {
			return;
		}
