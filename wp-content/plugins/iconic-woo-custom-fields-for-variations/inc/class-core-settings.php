...
if ( class_exists( 'Iconic_CFFV_Core_Settings' ) ) {
	return;
}

/**
 * Iconic_CFFV_Core_Settings.
 *
 * @class    Iconic_CFFV_Core_Settings
 * @version  1.0.6
 * @category Class
 * @author   Iconic
 */
class Iconic_CFFV_Core_Settings {
	/**
	 * Single instance of the Iconic_CFFV_Core_Settings object.
	 *
	 * @var Iconic_CFFV_Core_Settings
	 */
	 ...
	 *
	 * @var Iconic_CFFV_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_CFFV_Core_Settings object.
	 *
	 * @return Iconic_CFFV_Core_Settings
	 */
	...
	public static function init() {
		...
		self::$settings_framework = new Iconic_CFFV_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function account_getting_started() {
		...
		<div class="iconic-notice iconic-notice--getting-started">
			<form action="" method="post" class="iconic-notice__dismiss">
				...
				<button title="<?php echo esc_attr( __( 'Dismiss', 'iconic-cffv' ) ); ?>">
					<span class="dashicons dashicons-dismiss"></span></button>
			</form>
			<h2>Welcome to <?php echo self::$args['title']; ?>!</a></h2>
			<p><?php _e( "Thank you for choosing Iconic. We've put together some useful links to help you get started:", 'iconic-cffv' ); ?></p>

			<?php self::output_getting_started_links(); ?>

			<p><strong><?php _e( 'Is something not quite right?', 'iconic-cffv' ); ?></strong> <?php printf( __( 'Take a look at our
				<a href="%s" target="_blank">troubleshooting documentation</a>', 'iconic-cffv' ), self::get_docs_url( 'troubleshooting' ) ); ?>. <?php _e( 'There is a permanent link to the plugin documentation in the "Support" section below.', 'iconic-cffv' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public static function output_getting_started_links() {
		...
		<h3><?php _e( 'Getting Started', 'iconic-cffv' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_attr( self::get_docs_url() . $link['href'] ); ?>?utm_source=iconic-cffv&utm_medium=insideplugin" target="_blank"><?php echo $link['title']; ?></a>
				</li>
			<?php } ?>
		</ol>
	<?php }
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => __( 'Dashboard', 'iconic-cffv' ),
		);
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) ) {
			$settings['sections']['licence'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => __( 'License &amp; Account Settings', 'iconic-cffv' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'licence',
						'title'    => __( 'License &amp; Billing', 'iconic-cffv' ),
						'subtitle' => __( 'Activate or sync your license, cancel your subscription, print invoices, and manage your account information.', 'iconic-cffv' ),
						'type'     => 'custom',
						'default'  => Iconic_CFFV_Core_Licence::admin_account_link(),
					),
					array(
						'id'       => 'account',
						'title'    => __( 'Your Account', 'iconic-cffv' ),
						'subtitle' => __( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', 'iconic-cffv' ),
						'type'     => 'custom',
		...
		$settings['sections']['support'] = array(
			'tab_id'              => 'dashboard',
			'section_id'          => 'support',
			'section_title'       => __( 'Support', 'iconic-cffv' ),
			'section_description' => '',
			'section_order'       => 30,
			'fields'              => array(
				array(
					'id'       => 'support',
					'title'    => __( 'Support', 'iconic-cffv' ),
					'subtitle' => __( 'Get premium support with a valid license.', 'iconic-cffv' ),
					'type'     => 'custom',
					'default'  => self::support_link(),
				),
				array(
					'id'       => 'documentation',
					'title'    => __( 'Documentation', 'iconic-cffv' ),
					'subtitle' => __( 'Read the plugin documentation.', 'iconic-cffv' ),
					'type'     => 'custom',
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) ) {
			if ( ! empty( self::$args['cross_sells'] ) ) {
				$cross_sells = Iconic_CFFV_Core_Cross_Sells::run( array(
				...
				$settings['sections']['cross-sells'] = array(
					...
					'section_title'       => __( 'Works Well With...', 'iconic-cffv' ),
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support?utm_source=iconic-cffv&utm_medium=insideplugin', __( 'Submit Ticket', 'iconic-cffv' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), __( 'Read Documentation', 'iconic-cffv' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/account?utm_source=iconic-cffv&utm_medium=insideplugin', __( 'Manage Your Account', 'iconic-cffv' ) );
	}
}