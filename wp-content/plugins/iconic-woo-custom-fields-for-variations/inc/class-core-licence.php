...
if ( class_exists( 'Iconic_CFFV_Core_Licence' ) ) {
	return;
}

/**
 * Iconic_CFFV_Core_Licence.
 *
...
class Iconic_CFFV_Core_Licence {
	/**
	 * Single instance of the Iconic_CFFV_Core_Licence object.
	 *
	 * @var Iconic_CFFV_Core_Licence
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_CFFV_Core_Licence object.
	 *
	 * @return Iconic_CFFV_Core_Licence
	 */
	...
	public static function back_to_settings_link( $html = '' ) {
		return $html . sprintf( '<a href="%s" class="button button-secondary">&larr; %s</a>', self::$args['urls']['settings'], __( 'Back to Settings', 'iconic-cffv' ) );
	}
	...
	public static function output_back_to_settings_link() {
		if ( ! Iconic_CFFV_Core_Settings::is_settings_page( '-account' ) ) {
			return;
		}

		?>
		<div style="margin: 20px 0 10px;">
			<?php echo self::back_to_settings_link(); ?>
		</div>
		<?php
	}
	...
	public static function add_action_links( $links ) {
		$links[] = sprintf( '<a href="%s" target="_blank">%s</a>', self::$args['urls']['product'] . '/changelog/?utm_source=iconic-cffv&utm_medium=insideplugin', __( 'Changelog', 'iconic-cffv' ) );
	...
	public static function admin_account_link() {
		return sprintf( '<a href="%s" class="button button-secondary">%s</a>', self::$args['urls']['account'], __( 'Manage Licence &amp; Billing', 'iconic-cffv' ) );
	}
