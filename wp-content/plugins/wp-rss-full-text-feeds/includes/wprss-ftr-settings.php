...
class WPRSS_FTR_Settings {
	...
	public function __construct() {
		...
		if ( self::$instance !== NULL ) {
			wp_die( __('WPRSS_FTR_Settings class is singleton class, and cannot be instantiated more than once!', 'wprss') );
		}
	...
	public function add_dropdown_option( $services ) {
		$services['ftpr'] = __('Premium Full Text Service', 'wprss');
		return $services;
	}
	...
	public function check_dropdown_option_selectable( $selectables ) {
	    ...
	    $license = $manager->getLicense('ftr');

        $selectables['ftpr'] = ($license !== null) && $license->getStatus() === 'valid';
	...
	public function activate_license_callback() {
		...
		$valid = $status == 'valid';
		$btn_text = $valid ? __('Deactivate License', 'wprss') : __('Activate License', 'wprss');
