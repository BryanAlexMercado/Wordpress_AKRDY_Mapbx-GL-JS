...
class Jetpack extends Integration {
	 ...
	 *
	 * @todo Verify that Jetpack is present before attempting to load without using a hard-coded path.
	 */
	public function shouldLoadIntegration() {
		return $this->settings->is_mapps_site
			&& self::isPluginActive( 'jetpack/jetpack.php' )
	...
	/**
	 * Define JETPACK_STAGING_MODE in non-production environments.
	 */
	protected function setStagingMode() {
		if ( ! defined( 'JETPACK_STAGING_MODE' ) && ! $this->settings->is_production_site ) {
	...
	protected function getFilters() {
		...
		// Filters only applicable to Jetpack < 7.3.
		if ( version_compare( '7.3', $this->getJetpackVersion(), '>' ) ) {

			// Disable the "Jetpack Centralized Site Management" notice.
			$filters[] = [ 'can_display_jetpack_manage_notice', '__return_false' ];
		}
